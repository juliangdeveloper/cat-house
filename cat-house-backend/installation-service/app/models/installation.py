...
class Installation(Base, BaseModel):
    ...
    instance_name: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="User-defined instance name"
    )

    config: Mapped[Optional[dict]] = mapped_column(
        JSONB, nullable=True, comment="Installation-specific configuration (JSON)"
    )
    ...
    installed_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, comment="Installation timestamp (UTC)"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "cat_id", "instance_name", name="uq_user_cat_instance"
        ),
